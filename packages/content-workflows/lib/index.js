...
import { templateFor } from "@maomao/content-projects";
...
/** Loader plugin: provide the service, register Remotes, and the project-context section. */
export const name = "content-workflows";
export const inject = ["contentProjects", "typert", "systemPrompt", "agents"];

export function apply(ctx) {
	const service = new ContentWorkflowsService(ctx);
	ctx.effect(() => {
		return ctx.typert.register(buildContribution());
	}, "content-workflows: typert contribution");
	ctx.effect(() => {
		return ctx.systemPrompt.section({
			name: "maomao:project-context",
			order: 90,
			text: (context) => service.projectContextSection(context)
		});
	}, "content-workflows: project-context prompt section");
}
