window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var cm = null;
		var clientCtx = null;
		/** Resolve the current session id (view tab passes it; overlay falls back to the active session). */
		...
		function ContentDetail({ detail, setDetail, sessionId, setNote }) {
			...
			const startGenerate = (kind) => {
				setRunning(true);
				const call = kind === "subtitle" ? cm.generateSubtitles(item.id, resolveSessionId(sessionId)) : cm.generateCovers(item.id, resolveSessionId(sessionId));
				call.then(() => {
		...
		function ContentManagerWorkspace({ sessionId, initialId }) {
			...
			(0, react.useEffect)(() => {
				...
				if (typeof initialId === "string" && initialId !== "") {
					openItem(initialId);
				}
			}, []);
			...
			const startGenerate = (kind) => {
				...
				cm.get(id).then(() => {
					const call = kind === "subtitle" ? cm.generateSubtitles(id, resolveSessionId(sessionId)) : cm.generateCovers(id, resolveSessionId(sessionId));
					call.then(() => {
						const timer = window.setInterval(() => {
							cm.get(id).then((r) => {
								const run = r.run ?? null;
				...
				children: [
					...
					(0, react_jsx_runtime.jsx)("aside", {
						...
						children: (0, react_jsx_runtime.jsx)(AgentContextPanel, {
							...
							onOpenChat: () => {
								if (typeof window.__maomaoCloseWorkbench === "function") window.__maomaoCloseWorkbench();
							}
						})
					})
				]
		...
		// ── 左侧侧边栏入口：内容管理工作台浮层（Phase 6 三栏工作台）──
		var overlayStore = {
			open: false,
			initialId: null,
			...
			set(state) {
				...
				this.initialId = state?.initialId ?? null;
		...
		/** 全屏工作台浮层：三栏内容管理（左列表/中编辑/右 Agent 上下文）。 */
		function WorkbenchOverlay() {
			...
			const initialId = overlayStore.initialId;
			if (!open) return null;
			// 左侧停靠面板（参考 oil-creator：检查器叠在对话左边，聊天不关）。
			return (0, react_jsx_runtime.jsxs)("div", {
				className: "mxOverlayRoot",
				style: { left: `${sidebarChrome.collapsed ? 56 : sidebarChrome.width}px` },
				children: [
					(0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							(0, react_jsx_runtime.jsx)("div", {
								...
								children: "内容管理 · Creator Workbench"
							}),
							(0, react_jsx_runtime.jsx)("button", {
								type: "button",
								className: "mxOverlayClose",
								onClick: () => overlayStore.set({ open: false }),
								children: "关闭"
							})
						]
					}),
					(0, react_jsx_runtime.jsx)("div", {
						...
						children: (0, react_jsx_runtime.jsx)(ContentManagerWorkspace, { initialId })
					})
				]
			});
		}

		/** 左侧侧边栏（参考 oil-creator）：会话 | 内容管理 双 Tab。
		 * 会话 Tab 复用原生 sidebar.workspaces occupant；内容管理 Tab 是我们的
		 * 内容资产库，点击在 shell.overlay 中打开三栏工作台。 */
		function MaomaoSidebar(props) {
			const renderSlot = props.renderSlot;
			const collapsed = props.collapsed === true;
			const [tab, setTab] = (0, react.useState)("sessions");
			const [contentItems, setContentItems] = (0, react.useState)([]);
			const [note, setNote] = (0, react.useState)(null);
			const refreshContent = () => {
				if (cm === null) return;
				cm.list().then((r) => setContentItems(r.items)).catch(() => {
				});
			};
			(0, react.useEffect)(() => {
				if (cm !== null) refreshContent();
			}, []);
			const openContent = (id) => {
				cm.setCurrent(id).catch(() => {
				});
				overlayStore.set({ open: true, initialId: id });
			};
			const createContent = () => {
				if (cm === null) return;
				cm.create({ title: "未命名内容", type: "video" }).then((r) => {
					refreshContent();
					openContent(r.item.id);
				}).catch((e) => setNote(e instanceof Error ? e.message : String(e)));
			};
			const wide = !collapsed;
			(0, react.useEffect)(() => {
				setSidebarChrome(wide ? (typeof props.width === "number" && props.width > 0 ? props.width : 280) : 56, collapsed);
			}, [wide, collapsed, props.width]);
			const toggleSidebar = () => {
				try {
					clientCtx?.get("layout")?.toggleSidebar();
				} catch {
				}
			};
			const startSession = () => {
			...
			const railTabBtn = (label, title, active) => (0, react_jsx_runtime.jsx)("button", {
				type: "button",
				className: "mxSbarRailTab",
				"data-active": active || void 0,
				onClick: () => setTab(title === "会话" ? "sessions" : "content"),
				title,
				children: label
			});
			if (collapsed) {
				return (0, react_jsx_runtime.jsxs)("div", {
					className: "mxSbarRail",
					children: [
						railTabBtn("会", "会话", tab === "sessions"),
						railTabBtn("内", "内容管理", tab === "content"),
						(0, react_jsx_runtime.jsx)("div", { className: "mxSbarRailFoot", children: renderSlot("sidebar.settings", { wide: false }) })
					]
				});
			}
			return (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					...
					(0, react_jsx_runtime.jsxs)("div", {
						className: "mxSbarTabs",
						children: [
							(0, react_jsx_runtime.jsx)("button", {
								type: "button",
								className: "mxSbarTab",
								"data-active": tab === "sessions" || void 0,
							...
							(0, react_jsx_runtime.jsx)("button", {
								type: "button",
								className: "mxSbarTab",
								"data-active": tab === "content" || void 0,
								onClick: () => setTab("content"),
								children: "内容管理"
							})
						]
					}),
					tab === "sessions" ? (0, react_jsx_runtime.jsx)("div", {
						className: "mxSbarBody",
						...
						children: [
							(0, react_jsx_runtime.jsxs)("div", {
								className: "mxSbarCHead",
								children: [
									(0, react_jsx_runtime.jsx)("span", { className: "mxSbarCTitle", children: "内容资产" }),
									(0, react_jsx_runtime.jsx)("button", {
										type: "button",
										className: "mxSbarNew",
							...
							contentItems.map((item) => (0, react_jsx_runtime.jsxs)("div", {
								className: "mxSbarCItem",
								onClick: () => openContent(item.id),
								children: [
									(0, react_jsx_runtime.jsx)("div", { className: "mxSbarCItemTitle", children: item.title }),
									(0, react_jsx_runtime.jsx)("div", { className: "mxSbarCItemMeta", children: `${CONTENT_TYPE_LABELS[item.type] ?? item.type} · ${CONTENT_STATUS_LABELS[item.status] ?? item.status}` })
								]
							}, item.id)),
							contentItems.length === 0 && (0, react_jsx_runtime.jsx)("div", { className: "mxSbarCNote", children: "暂无内容。点「＋ 新建」创建视频/脚本/字幕/封面/文章/发布。" }),
							note !== null && (0, react_jsx_runtime.jsx)("div", { className: "mxSbarCNote", children: note })
						]
					}),
					(0, react_jsx_runtime.jsxs)("div", {
						className: "mxSbarFoot",
						children: [
		...
		function apply(ctx) {
			...
			window.__maomaoCloseWorkbench = () => overlayStore.set({ open: false });

			// 左侧侧边栏：Tab（会话 | 内容管理），参考 oil-creator 做法。
